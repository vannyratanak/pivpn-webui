...
def create_server(name: str) -> tuple[int, str]:
    ...
    try:
        try:
            cur = conn.execute(
                "INSERT INTO servers (name, token_hash) VALUES (%s, %s) RETURNING id",
                (name, generate_password_hash(token)),
            )
        except psycopg2.errors.UniqueViolation as exc:
            conn.rollback()
...
def verify_server_token(server_id: int, token: str) -> bool:
    conn = get_conn()
    try:
        row = conn.execute("SELECT token_hash FROM servers WHERE id=%s", (server_id,)).fetchone()
        return bool(row and check_password_hash(row["token_hash"], token))
    finally:
        conn.close()
